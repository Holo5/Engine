...
export class CornerWallTexture extends WallGraphicTexture {
    ...
    private drawRightBorder() {
        ...
        const rightBorders = new PIXI.Container();
    ...
    private drawLeftBorder() {
        ...
        const leftBorders = new PIXI.Container();
        const leftTopBorder = new PIXI.Graphics()
            .beginFill(0x989898)
        ...
        leftTopBorder.tint = PIXI.utils.premultiplyTint(this._wallMaterial.color, 0.6);
        const leftLeftBorder = new PIXI.Graphics()
            .beginFill(0xFFFFFF)
            .moveTo(this._points[0].x - this._wallThickness * 2, this._points[0].y - this._wallThickness * 2)
            .lineTo(this._points[0].x, this._points[0].y)
        ...
        leftTopBorder.tint = PIXI.utils.premultiplyTint(this._wallMaterial.color, 1);
    ...
    public drawParts(): void {
        ...
        const leftTexturedPart = new PIXI.TilingSprite(this._wallMaterial.texture, 32, wallSize);
        leftTexturedPart.transform.setFromMatrix(Engine.RIGHT_ISO_MATRIX);
        leftTexturedPart.tint = PIXI.utils.premultiplyTint(this._wallMaterial.color, 0.85);

        const rightTexturedPart = new PIXI.TilingSprite(this._wallMaterial.texture, 32, wallSize);
        rightTexturedPart.transform.setFromMatrix(Engine.LEFT_ISO_MATRIX);
        rightTexturedPart.tint = PIXI.utils.premultiplyTint(this._wallMaterial.color, 1.1);
